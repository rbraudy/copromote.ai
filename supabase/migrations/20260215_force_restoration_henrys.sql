-- FINAL RESTORATION: Sync Henry's hand-tuned script from index.ts.bak to the correct company ID
DO $$
DECLARE
    v_target_id UUID := 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'; -- The "Henry's Camera" ID from Superadmin Oversight
    v_script TEXT;
BEGIN
    -- This is the exact script from the "source of truth" backup index.ts.bak
...
3. ** The Close:**
   ...
   - ** Ask if there any other questions you can answer.**
...
8. Tools: Use ''sendSms''.The message MUST technically follow this exact format: "Hi {{customer_name}}! We''ve activated 7 days of the Henry''s Extended Warranty Protection for your {{product_name}} at no charge. This covers common issues like shutter motor failures, 30 day price protection, and over the counter replacements. You can view all the features of the plan here: {{link}}"';
    ...
    VALUES (v_target_id, v_script, now())
