...
DECLARE
    ...
    v_prompt TEXT;
BEGIN
    -- 1. Get Henry's Company ID
    SELECT id INTO v_company_id FROM public.companies WHERE name = 'Henry''s' LIMIT 1;

    IF v_company_id IS NULL THEN
        RAISE NOTICE 'Henry''s company not found. Skipping seed.';
        RETURN;
    END IF;

    -- 2. Construct the System Prompt (Mustache Syntax)
    v_prompt := 'You are {{agent_name}}, a Henry''s Warranty Expert. You''re calling {{customer_name}} about their recent purchase of {{product_name}}.
...
- ** PHASE 1: Your first message "Hi! ... Is {{customer_name}} there?" has ALREADY been spoken. Wait for the customer to confirm they are on the line. **
...
- ** Response **: "I''ll be very brief then. I''m calling because we''ve gifted you 7 days of Henry''s Extended Protection at no charge on your new {{product_name}}—mostly for peace of mind."
...
- "That is great to hear. Just to help us tailor our future support for you—are you primarily using this new {{product_name}} for professional work, or is it more for personal projects?"
...
8. Tools: Use ''sendSms''.The message MUST technically follow this exact format: "Hi {{customer_name}}! We''ve activated 7 days of the Henry''s Extended Warranty Protection for your {{product_name}} at no charge. Pricing starts at just $12/mo or ${{warranty_price_2yr}} for 2 years. This covers common issues like shutter motor failures, 30 day price protection, and over the counter replacements. You can view all the features of the plan here: {{link}}"';
    ...
    -- 3. Insert into call_templates
    INSERT INTO public.call_templates (company_id, system_prompt, first_message, voice_id, tools_config)
    VALUES (
        v_company_id,
        v_prompt,
        'Hi! ... Is {{customer_name}} there?',
        'jBzLvP03992lMFEkj2kJ', -- Paige ID from Vapi
        '["sendSms", "reportIssue", "offerDiscount"]'::jsonb
    )
    ...
    -- 4. Seed Integrations placeholders
    INSERT INTO public.integrations (company_id, provider, credentials, is_enabled)
    VALUES 
        (v_company_id, 'shopify', '{"apiKey": "placeholder", "shop": "henrys.myshopify.com"}'::jsonb, false),
        (v_company_id, 'eshipper', '{"username": "placeholder", "password": "placeholder"}'::jsonb, false)
    ON CONFLICT (company_id, provider) DO NOTHING;

    RAISE NOTICE 'Henry''s template seeded successfully.';
END $$;
